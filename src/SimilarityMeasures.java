...
public class SimilarityMeasures {
	...
	private static int correctJacPairsW = 0, correctCosinePairsW = 0;
	...
	public static void main(String[] args) {
		...
		try {
			...
			double wScorecosine = ((double) correctCosinePairsW) / testFreq * 100;
			...
			// System.out.println("Jaccard Weight score weight: " +
			// wScoreJaccardW);
			...
			// System.out.println("cosine score weight: " + wScorecosine);
		...
		try {
			...
			if (isCorrect) {
				...
				if (simType.equals("jaccard")) {
					...
					correctCosinePairs++;
					correctCosinePairsW += oov.getFrequency();
				} else if (simType.equals("eu")) {
					correctEuPairs++;
		...
		Collections.sort(similarity, new Comparator<Word>() {
			...
			public int compare(Word arg0, Word arg1) {
				if (simType.equals("jaccard")) {
					...
					return Double.compare(arg0.editDistance, arg1.editDistance);
				} else {
					return Double.compare(arg1.lcs, arg0.lcs);
				}
			}
		});
	}
	...
	public static void addResult(ArrayList<Word> similarity, String simType, double result, String iv) {
		...
		if (simType.equals("jaccard")) {
			...
			last = similarity.get(similarity.size() - 1).editDistance;
		} else if (simType.equals("lcs")) {
			last = similarity.get(similarity.size() - 1).lcs;
		}
		
		if ((simType.equals("eu") || simType.equals("edit")) && result < last) {
			Word w = new Word(iv);
			if (simType.equals("eu")) {
				w.setEuclidean(result);
			} else if (simType.equals("edit")) {
				w.editDistance = result;
			} 
